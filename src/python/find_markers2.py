...
class MarkerFinder:
    ...
    def find_X_markers(self, tf):
        ...
        tfX['bg_delta_min'] = tfX[self.bg_names].min(axis=1)
        tfX['tg_delta_max'] = tfX[self.tg_names].max(axis=1)
        tfX['bg_delta_max'] = tfX[self.bg_names].max(axis=1)
        tfX['tg_delta_min'] = tfX[self.tg_names].min(axis=1)
